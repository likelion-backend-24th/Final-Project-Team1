...
export default function ExpoManagePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    if (!channelId) {
      setError('채널을 먼저 생성해주세요.')
      return
    }
    setLoading(true)
    try {
      const res = await expoApi.createExpo(channelId, form)
      toast('박람회가 등록되었습니다 (HIDDEN 상태)', 'success')
      ...
      <div className="card" style={{ maxWidth: 640, padding: 32 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <textarea
              className="form-input"
              placeholder="박람회에 대한 소개를 입력하세요"
              value={form.description}
              onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
            />
          </div>

          <button type="submit" className="btn btn-primary btn-block" disabled={loading || !channelId}>
            {loading ? '등록 중...' : '박람회 등록'}
          </button>
        </form>
      </div>
