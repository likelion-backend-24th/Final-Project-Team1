...
-- 사용 방법: 이 내용을 그대로 복사해서, payment_transactions.sql 을 이미 적용한 각 서비스
-- (Reservation-Service, 박람회-Service) 의 Flyway 마이그레이션 폴더에 다음 버전으로 붙여넣는다.
--
-- 환불 실패 재시도(#125). REFUND_FAILED 로 남은 건을 배치가 회수할 수 있도록
-- attempts·next_attempt_at 을 payment_transactions 에 직접 추가한다 - 별도 큐 테이블을
-- 두지 않는다. 재시도 대상은 반드시 status = REFUND_FAILED 로만 잡는다 - 예약 취소가
-- 환불 기한을 지나 PAID 로 남는 조합은 정상 상태라, 그 행까지 쓸어담으면 안 된다.
